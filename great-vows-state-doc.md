...
- **Lunch slot** wired to midday service (`midday-service` from sfzc.json) — resolves `_middaySvc` day-invariantly, calls `loadService(ms, day.key)`, shows chant titles via `_slotTitles()`. `lunch-placeholder` CSS class retired from markup (CSS rules remain, inert). `_serviceKind === 'midday'` branch added to both `loadService()` and the `?service=` deep-link handler — displays "Midday Service" title.
- **schedule.html `#enterBtn`** extended to midday window: `midday-service` now has `hasService: true`; `serviceId` assigned as `'midday-service'` (day-invariant, unlike morning/evening which use `resolveServiceId()`); `'midday-service'` entry added to `SERVICE_AUDIO` pointing at `midday-service.mp3`. The existing `updateEnterBtnElapsed()` and `updateSidebar()` logic handles elapsed `?t=` seek and the `&day=` param automatically. No structural duplication — the existing condition generalizes to any `hasService: true` period with a SERVICE_AUDIO entry.
...
1. **Pre-first-chant text leak** — ✅ **FIXED (May 2026).** `openChant` previously called `setClasses(firstTextIndex())` in the double-RAF, which pre-activated the first ceremony cue (C-0) from t=0. Now: when the chant has `ceremonyCues`, `openChant` sets `currentIndex = -1` and lets the tween engine take over. Display is blank before first cue fires.
2. **Last-line dims too early** — ✅ **FIXED (May 2026).** `chantEndTimes` now uses `nextEntry.startTime` for interior chants (not `lastCueIn + 3`). New `chantEndOverrides` field in sfzc.json for explicit per-service overrides (e.g. `after-dedication-japanese: 938` on Wed). Precedence: `chantEndOverrides[chantId]` → next-chant `startTime` → `svc.duration` (last chant) → `lastCueIn + 10` (defensive only). `lastCueIn + 3` removed as a primary path.
...
- **Reciprocal link, the real fix.** Diagnosis: schedule→index was *only* the
  ...
  Service *(stale — `#enterBtn` now appears during morning, midday, and evening
  service windows; the ~30 min / morning-only framing is retired)*; on mobile
  `.sidebar-header` is `display:none` so the SFZC anchor vanished too. Decision: **keep `#enterBtn` exactly as is** (it's the contextual
...
- **`chantEndOverrides`** — service-level field (parallel to `timestampCorrections`); object keyed by chantId, values in service-elapsed seconds. Overrides the renderer's end-time for a chant (used when next-chant-start is wrong — e.g. last chant before closing bows). Human-authored, never written by aligner. Example: `"chantEndOverrides": {"after-dedication-japanese": 938}` on Wed.
...
### Service status (SFZC evening + midday services)
- **Evening service** — added (Dai Hi Shin Darani + after-dedication-english). Substantially complete. See addendum below.
- **Midday service** — added (Self-Receiving and Employing Samadhi + after-dedication-english). `--model small` aligned in 15s; self-employing-samadhi clean (50.9s start, 450.3s end). after-dedication-english drifted ~66s early (eko-drift — see rule below); corrected via `timestampCorrections` using evening service gap spacing (+13s/+20s/+13s from 537s onset). Substantially complete.

**Eko-drift rule (fixture of any service with a doshi eko gap):** `stable_whisper` forced alignment has no knowledge of the unscripted doshi eko between the main chant and after-dedication-english. It places after-dedication-english inside the eko gap, consistently early by the eko duration. Fix: get the onset of line 0 by ear, apply evening-service gap spacing (±1s) for lines 1–3 via `correct_timestamp.py`. This will recur on every new service with a doshi eko.
...
- **Opening/closing bow ceremonial cues** for Wed/Thu — bow timing cues (densho, kinhin, prostrations) not yet authored. Separate focused listen, different concentration than sync drift. (Thu announcement cues and last-line tail now complete — closed this pass.)
...
### Time-travel debug tool — Time Machine 2.0 — working
...
Tap opens `#tt-panel` — 268px floating panel (flex-column). Contains: large clock display (`#tt-clock`), period + elapsed info (`#tt-period-info`), scrub track (`#tt-scrub-track` with fill + thumb), snap indicator (`#tt-snap-indicator`), cue row (`‹ #tt-cue-name / #tt-cue-nav ›`), and Go button.

**Scrub track** — drag horizontally to move through the day (00:00–24:00). Velocity-sensitive: slow drag snaps to nearest `_CUE_LIST` entry within 15 min window; fast drag (`dragVelocity ≥ 4px`) moves freely. Snap indicator shows `· cue` or `· free`. Touch events supported.

**Cue step buttons** — `‹ ›` step through `_CUE_LIST` one event at a time. Each step fires the cue immediately (`getNow() + 500ms`) so the practitioner hears the audio at the transition point.

**`_CUE_LIST`** — flat sorted array of every scheduled event for the day, built inside `scheduleUpcomingBells()` unconditionally (even without `_webAudioCtx`). Fields: `{ fireMs, type, src, label, periodId }`. Types: `bell`, `bellEnd`, `service`, `densho`, `kinhin`, `chant-offset`. Published as `window._CUE_LIST`. Service cues appear for navigation but are not scheduled via `scheduleAudioEvent` (still fired by `tickServiceAudio`).

**Ghost bell fix** — `_scheduledTimeouts` array accumulates all `setTimeout` IDs from `scheduleAudioEvent`. Cleared with `_scheduledTimeouts.forEach(clearTimeout)` at the top of `scheduleUpcomingBells()`. Eliminates stale bells firing after time-travel cancel.

On Go: `_debugOffset = _ttScrubTargetMs - Date.now()`, stops all audio, rebuilds track, reschedules bells.
On cancel (tap button while active): same audio teardown as Go, clears `_debugOffset`, rebuilds/reschedules.
...
- ~~Time-travel ghost bell cancellation~~ ✅ fixed: `_scheduledTimeouts` array accumulates all setTimeout IDs; cleared at top of `scheduleUpcomingBells()`.
